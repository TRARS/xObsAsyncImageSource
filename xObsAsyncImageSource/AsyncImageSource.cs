...
using xObsAsyncImageSource.Wrapper;
...
public partial class AsyncImageSource
{
    public unsafe struct image_source
    {
    ...
    static unsafe void image_source_update(void* data, obs_data* settings)
    {
        ...
        fixed (byte*
          ...
          propertyFeatureIdentifier = "feature"u8
        )
        {
            //var feature = Proxy.Bytes2String(ObsData.obs_data_get_string(settings, (sbyte*)propertyFeatureIdentifier));
            var feature = Proxy.Bytes2String(context->feature);

            Proxy.SetSettings(feature, settings); // 实时更新
            Proxy.SetContext(feature, context);
    ...
    {
        var msg = "image_source_show called";

        Module.Log(msg, ObsLogLevel.Debug);
        var context = (image_source*)data;
    ...
    {
        var msg = "image_source_hide called";

        Module.Log(msg, ObsLogLevel.Debug); //Proxy.MessageBoxTask(msg);
    ...
    static unsafe void* image_source_create(obs_data* settings, obs_source* source)
    {
        ...
        var featureString = RandomStringGenerator.GetRandomString(8);
        var featureBytes = Proxy.String2Bytes(featureString);
        Array.Resize(ref featureBytes, featureBytes.Length + 1); // 扩展数组以放置终止符
        featureBytes[featureBytes.Length - 1] = 0; // 设置终止符为 '\0'

        // 创建时生成特征码
        fixed (byte* propertyFeatureIdentifier = "feature"u8)
        {
            var feature = Proxy.CreatePinnedObject(featureBytes);

            ObsData.obs_data_set_string(settings, (sbyte*)propertyFeatureIdentifier, feature);
            Proxy.SetSettings(featureString, settings); // 首次更新
    ...
    static unsafe void image_source_destroy(void* data)
    {
        ...
        image_source_unload(context);
        Proxy.TryLogout(Proxy.Bytes2String(context->feature));
    ...
    static unsafe obs_properties* image_source_get_properties(void* data)
    {
        ...
        fixed (byte*
          ...
          propertyFeatureIdentifier = "feature"u8,
          propertyFeatureCaption = Module.ObsText("Feature"),
          propertyServerIPIdentifier = "server_ip"u8,
          propertyServerIPCaption = Module.ObsText("ServerIP"),
          propertyServerPortIdentifier = "server_port"u8,
          propertyServerPortCaption = Module.ObsText("ServerPort"),
          propertyChatMessageIdentifier = "chat_message"u8,
          propertyChatMessageCaption = Module.ObsText("ChatMessage"),
          propertyLoginIdentifier = "login"u8,
          propertyLoginCaption = Module.ObsText("Login"),
          propertySendMessageIdentifier = "send_message"u8,
          propertySendMessageCaption = Module.ObsText("SendMessage"),
      ...
      "u8
        ...
        )
        {
            ...
            prop = ObsProperties.obs_properties_add_text(properties, (sbyte*)propertyFeatureIdentifier, (sbyte*)propertyFeatureCaption, obs_text_type.OBS_TEXT_INFO);
            prop = ObsProperties.obs_properties_add_list(properties, (sbyte*)propertyServerIPIdentifier, (sbyte*)propertyServerIPCaption, obs_combo_type.OBS_COMBO_TYPE_EDITABLE, obs_combo_format.OBS_COMBO_FORMAT_STRING);
            {
                foreach (var ip in LocalAddressRepository.LocalIPs)
                {
                    if (ip.AddressFamily == System.Net.Sockets.AddressFamily.InterNetwork)
                    {
                        fixed (byte* item = Proxy.String2Bytes($"{ip}"))
                        {
                            ObsProperties.obs_property_list_add_string(prop, (sbyte*)item, (sbyte*)item);
                        }
                    }
                }
            }
            prop = ObsProperties.obs_properties_add_list(properties, (sbyte*)propertyServerPortIdentifier, (sbyte*)propertyServerPortCaption, obs_combo_type.OBS_COMBO_TYPE_EDITABLE, obs_combo_format.OBS_COMBO_FORMAT_STRING);
            prop = ObsProperties.obs_properties_add_button(properties, (sbyte*)propertyLoginIdentifier, (sbyte*)propertyLoginCaption, &func_login);
            prop = ObsProperties.obs_properties_add_list(properties, (sbyte*)propertyChatMessageIdentifier, (sbyte*)propertyChatMessageCaption, obs_combo_type.OBS_COMBO_TYPE_EDITABLE, obs_combo_format.OBS_COMBO_FORMAT_STRING);
            prop = ObsProperties.obs_properties_add_button(properties, (sbyte*)propertySendMessageIdentifier, (sbyte*)propertySendMessageCaption, &func_send);



            // 弹窗时读取一下特征码
            Proxy.SetCurrentFeature(Proxy.Bytes2String(s->feature));
        }
        return properties;
    }
...
public partial class AsyncImageSource
{
    ...
    static unsafe byte func_login(obs_properties* props, obs_property* prop, void* data)
    {
        if (isTaskRunning is false)
        {
            ...
            Task.Run(() =>
            {
                var settings = Proxy.GetSettings(); // 读取Settings
                ...
                {
                    var feature = Proxy.Bytes2String(ObsData.obs_data_get_string(settings, (sbyte*)propertyFeatureIdentifier));
                    var server_ip = Proxy.Bytes2String(ObsData.obs_data_get_string(settings, (sbyte*)propertyServerIPIdentifier));
                    var server_port = Proxy.Bytes2String(ObsData.obs_data_get_string(settings, (sbyte*)propertyServerPortIdentifier));

                    var ip = string.IsNullOrWhiteSpace(server_ip) ? null : server_ip;
                    int? port = int.TryParse(server_port, out var temp) ? temp : null;

                    Proxy.TryLogin(feature, ip, port, path =>
                    {
                        ...
                        var file = File.Exists(path) ? path : null;

                        if (file is not null)
                        {
                            fixed (byte* propertyFileIdentifier = "file"u8, filePath = Proxy.String2Bytes(file))
                            {
                                // 写入属性
                                ObsData.obs_data_set_string(settings, (sbyte*)propertyFileIdentifier, (sbyte*)filePath);
    ...
    [UnmanagedCallersOnly(CallConvs = [typeof(System.Runtime.CompilerServices.CallConvCdecl)])]
    static unsafe byte func_send(obs_properties* props, obs_property* prop, void* data)
    {
        if (isTaskRunning is false)
        {
            isTaskRunning = true;
            ...
            Task.Run(() =>
            {
                ...
                {
                    var feature = Proxy.Bytes2String(ObsData.obs_data_get_string(settings, (sbyte*)propertyFeatureIdentifier));
                    var chat_message = Proxy.Bytes2String(ObsData.obs_data_get_string(settings, (sbyte*)propertyChatMessageIdentifier));

                    // 发送消息
                    Proxy.TrySendMessage(feature, chat_message);
                }
